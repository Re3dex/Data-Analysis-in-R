...
author: 'Name: Ammar Khalid'
date: 'Roll Number: 241547086'
output:
...
---
## Table of Contents
1. Creating the complete Dataset
2. Calculating percentage of missing data
3. Checking and Removing the Outliers in the data
4. The Functions
5. The data analysis

## 1. Creating the complete Dataset

To do this, I had to install readxl package in R. I also had to rename the column names in excel as some had "Pop" while others had "pop" which R recognizes as two different things entirely. Make sure the xlsx files are in your working directory to ensure they are read by the read_excel function.
```{r readingxl, include=FALSE}
library(readxl)
...
# Binding all the data into one variable 
...
# Reading and saving the data 
# Counties_Data_Set_1<-read_excel("Counties Data Set 1.xlsx") - Reads from File 1
# Counties_Data_Set_2<-read_excel("Counties Data Set 2.xlsx") - Reads from File 2
# Counties_Data_Set_3<-read_excel("Counties Data Set 1.xlsx") - Reads from File 3

# Binding all the data into one variable 
# mydata<-rbind(Counties_Data_Set_1,Counties_Data_Set_2,Counties_Data_Set_3) - combines the data
```
...
I will be using a boxplot to check for outliers and any clear outliers will be replaced with the average of the column to avoid other data from being skued. Non-Apparent outliers will be kept in.

```{r outlier}
boxplot(mydata$Pop,horizontal=TRUE) # Original Boxplot
#Outliers exist above 3x10^6
mean1<-mean(Pop)
mydata$Pop[mydata$Pop>3000000]<-mean1 #Replaces outliers with mean
boxplot(mydata$Pop,horizontal=TRUE) # New Boxplot
...
This step was repeated for every variable of the data so I will not be showing it over and over.
...
## 5. The data analysis
The data was run through the "my_summary" function writted above and stored. This was done for every variable so I will not be showing all of the procedure here.
...
The final results were then generated.
...
Bar Plots were also generated as follows:
```{r barplots}
...
```
Table for the average and standard deviation for the variables was also generated along with plots of percentages of White and Black in each state. Functions from the libraries data.table and ggplot2 were used.

```{r inits, include=FALSE}
library(ggplot2)
library(data.table)
```
